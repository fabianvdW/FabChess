        ...
        //Remove enpassented pawn
            ...
            if self.color_to_move == WHITE {
                if mv.to == self.castle_target_square(true) {
                    if !self.is_valid_castle(true, true, &mut occ) {
                        return false;
                    }
                } else {
                    if !self.is_valid_castle(true, false, &mut occ) {
                        return false;
                    }
                }
            } else {
                if mv.to == self.castle_target_square(true) {
                    if !self.is_valid_castle(false, true, &mut occ) {
                        return false;
                    }
                } else {
                    if !self.is_valid_castle(false, false, &mut occ) {
                        return false;
                    }
                }
            }
